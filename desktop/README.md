...
5. 选择当前电脑网络并填写手机代理端口（默认 `55255`，可修改），按页面显示设置手机 Wi-Fi 手动代理，打开领克 App。电脑换 Wi-Fi 后，回到配对页刷新网络并点击「换 Wi-Fi 后重新配对」，旧二维码不会继续使用。
6. 获取完整登录态后，助手会静默验证个人信息；验证成功后确认云端绑定。
7. 关闭手机 Wi-Fi 代理，移除本次证书，再在助手中确认断开连接。电脑此后可以关机。

关闭浏览器不会退出后台；启动器窗口会持续显示后台程序和手机请求活动，重新双击启动器会复用正在运行的客户端并重新打开页面。关闭启动器会先静默停止电脑代理，再停止客户端；手机端 Wi-Fi 代理开关仍需用户自行关闭。使用页面右上角电源图标「退出助手」仍要求先完成网页中的断开步骤。页面刷新丢失连接时，重新双击应用即可。启动时先显示本机已保存的状态，再后台刷新云端；短暂网络故障不会清空当前页面，恢复网络后可点击「刷新」重试。

启动器显示下载进度，客户端页面打开后继续显示运行状态；启动器还提供「打开后台」和「清除本地登录态」按钮。清除按钮只删除本机系统凭据，不删除云端绑定，执行前会确认。macOS 下载包中是可双击的 `.app`，Windows 是窗口模式 `.exe`；从 SSH 或无图形环境直接运行时才回退到终端输出。强制结束或断电时可能留下临时资源，下次启动仅清理已退出进程的旧目录。账号管理凭据、本机 CA 和设置保留，不属于临时程序资源。首次启动或版本变化时下载资源；已验证的缓存会复用，下载时才需要能访问 GitHub 和其 Release 附件域名。
