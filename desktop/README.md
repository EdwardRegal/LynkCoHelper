...
启动器显示下载进度，确认客户端本地页面已经打开后自动关闭；客户端会继续独立运行，关闭这个进度窗口不会断开页面。macOS 下载包中是可双击的 `.app`，Windows 是窗口模式 `.exe`；从 SSH 或无图形环境直接运行时才回退到终端输出。使用客户端页面里的「退出助手」会停止本机服务并清理本次下载与解压目录。强制结束或断电时可能留下临时资源，下次启动仅清理已退出进程的旧目录。账号管理凭据、本机 CA 和设置保留，不属于临时程序资源。首次启动或版本变化时下载资源；已验证的缓存会复用，下载时才需要能访问 GitHub 和其 Release 附件域名。
