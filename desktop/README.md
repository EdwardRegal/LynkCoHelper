...
关闭浏览器不会退出后台；启动器窗口会持续显示客户端状态，重新双击启动器会复用正在运行的客户端并重新打开页面。使用页面右上角电源图标「退出助手」，或关闭启动器窗口，均会停止程序。页面刷新丢失连接时，重新双击应用即可。不要在手机代理开启时强制退出。启动时先显示本机已保存的状态，再后台刷新云端；短暂网络故障不会清空当前页面，恢复网络后可点击「刷新」重试。

启动器显示下载进度，客户端页面打开后继续显示运行状态；关闭启动器窗口会通知客户端退出。macOS 下载包中是可双击的 `.app`，Windows 是窗口模式 `.exe`；从 SSH 或无图形环境直接运行时才回退到终端输出。使用客户端页面里的「退出助手」也会停止本机服务并让启动器结束。强制结束或断电时可能留下临时资源，下次启动仅清理已退出进程的旧目录。账号管理凭据、本机 CA 和设置保留，不属于临时程序资源。首次启动或版本变化时下载资源；已验证的缓存会复用，下载时才需要能访问 GitHub 和其 Release 附件域名。
